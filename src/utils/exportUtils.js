 ...
 */
export function generateCSV(headers, data, delimiter = ',') {
  const formatCell = (value) => {
    if (value === null || value === undefined) return 'NA';
    let text = String(value);
    // Treat untrusted spreadsheet expressions as text, including after whitespace.
  ...
  const processedHeaders = headers.map(formatCell);
  const isMultiRow =
    Array.isArray(data) && data.length > 0 && Array.isArray(data[0]);
  const rows = isMultiRow ? data : [data];

  const processedRows = rows.map((row) =>
    (Array.isArray(row) ? row : [row]).map(formatCell).join(delimiter),
  );

  const headerRow = processedHeaders.join(delimiter);
  return `${headerRow}\n${processedRows.join('\n')}`;
}
